...
### 1. 🚀 시작 로딩 스플래시 화면 & 최상단 강제 팝업 (Splash Screen)
...
### 2. 🍞 부드러운 플로팅 토스트 알림 (Toast Notification)
- **비방해성 실시간 알림**: 데이터를 복사하거나 컬럼을 클릭할 때 화면 하단에 `📋 클립보드 복사 완료!` 팝업이 부드럽게 타올라 떴다가 1.8초 후 자동으로 사라짐 (모달 OK 창 없는 쾌적한 UX).

### 3. 📋 셀 드래그 블록 선택 & `Ctrl+C` 복사 (`CopyableTableWidget`)
- **엑셀식 자유 블록 선택**: 결과 테이블의 원하는 영역(특정 행/열 블록)을 마우스 드래그로 선택 후 `Ctrl+C`를 누르면 엑셀 탭 구분(TSV) 형식으로 즉시 복사.

### 4. 📊 엑셀 집합 비교 분석 (Set Analyzer 탭)
...
- 💡 **샘플 데이터 채우기**: 원클릭으로 테스트용 샘플 데이터 자동 채우기 지원.

### 5. 🔗 인터랙티브 컬럼 Concat / SQL 쿼리 생성기 (Column Concat 탭)
- **인터랙티브 테이블 헤더 클릭**: 엑셀 표 데이터 붙여넣기 시 원본 컬럼이 테이블에 자동 배치되며, 헤더(`[A]`, `[B]`, `[C]`)를 직접 클릭 클릭하여 병합 순서 지정.
...
├── gui.py                     # PySide6 데스크톱 GUI 메인 애플리케이션 (Toast, Table Copy, Concat, Preset 포함)
├── excel_processor.py         # pandas/openpyxl 기반 집합 연산 코어 엔진
├── AGENTS.md                  # 개발 에이전트 지침 및 UI 콘트라스트 규칙
└── README.md                  # 프로젝트 최신 설명 문서
```
